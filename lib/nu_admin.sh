...
# nuOS 0.0.9.2a24 - lib/nu_admin.sh - LICENSE: BSD_SMPL
#
...
admin_init () {
	...
	echo -n 'admin keys         ADMIN_KEYS      ' && [ -n "${ADMIN_ACCT-}" -a -n "${OPT_INSTALL_ADMIN_KEYS-}" ] && echo ${ADMIN_KEYS:=~$ADMIN_ACCT/.ssh/id_ecdsa.pub} || echo 'n/a (use -k to install keys)'
	...
	echo -n 'user keys          USER_KEYS       ' && [ -n "${USER_ACCT-}" -a -n "${OPT_INSTALL_ADMIN_KEYS-}" ] && echo ${USER_KEYS:=~$USER_ACCT/.ssh/id_ecdsa.pub} || echo 'n/a (use -k to install keys)'
	...
	echo -n 'backdoor keys      BD_KEYS         ' && [ -n "${BD_ACCT-}" -a -n "${OPT_INSTALL_ADMIN_KEYS-}" ] && echo ${BD_KEYS:=~$BD_ACCT/.ssh/id_ecdsa.pub} || echo 'n/a (use -k to install keys)'
}
